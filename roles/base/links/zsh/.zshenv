# By default, Zsh will try to find the following configuration files (in that order) in the user's $HOME directory (defined in `/etc/passwd` for each user) or in `/etc/zsh/` for all users.
#
