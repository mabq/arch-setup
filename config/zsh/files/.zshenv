# Zsh will look for the following configuration files (in that order) in the
# user's $HOME directory (defined in `/etc/passwd` for each user) or in
# `/etc/zsh/` for all users.
#
...
# will instruct zsh about where to look for config files.

# See a graphical representation here:
