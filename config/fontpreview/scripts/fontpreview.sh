...
# Modifies version of https://github.com/xlucn/fontpreview-ueberzug.
# The original script causes an issue because of `--no-opencv` option, see 
# below.
...
start_ueberzug() {
    ...
    mkfifo "$FIFO" || exit 1
    touch "$IMAGE" || exit 1
    # Must use `--no-openvs` to avoid "loading" (segmentation fault) error.
    ueberzugpp layer --no-cache --no-opencv --silent < "$FIFO" &
